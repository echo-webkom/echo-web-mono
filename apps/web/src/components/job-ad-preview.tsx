...
export const JobAdPreview = ({ jobAd, className }: JobAdPreviewProps) => {
  return (
    <Link href={`/for-studenter/stillingsannonse/${jobAd.slug}`} className="h-full">
      ...
      >
        {jobAd.company && (
          <div className="block">
            <div className="relative h-16 w-16 overflow-hidden rounded-full border bg-white sm:h-20 sm:w-20">
              <Image
        ...
        <div className="flex h-full w-full flex-col gap-1 overflow-x-hidden">
          ...
          <ul className="flex flex-wrap gap-3 gap-y-1 p-0 text-xs sm:text-sm">
            {jobAd.company && (
