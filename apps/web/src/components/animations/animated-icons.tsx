...
/**
 * Christmas version of the animated icons
 * @param n Number of icons
 * @param children
 * @returns animated icons
 */
export const AnimatedIconsChristmas = ({ n, children }: AnimatedIconsProps) => {
  const month = getMonth(new Date());

  if (month !== 12) return <>{children}</>;

  const keys = [...new Array(n).keys()];
  const image = "/christmas-icons/santa_hat.svg";
...
export const AnimatedIconChristmas = ({
  xOffset,
  yOffset,
  delay,
  repeatDelay,
