import { happeningTypeToPath, happeningTypeToString, registrationStatusToString } from "@echo-webkom/lib";
import Link from "next/link";
import { redirect } from "next/navigation";

import { unoWithAdmin } from "@/api/server";
import { RegistrationStatus, type UnoReturnType } from "@/api/uno/client";
    ...
    const pastRegistrations = registrations
        ...
        .filter(
            (registration) =>
                ...
                (new Date(registration.happening.date) < new Date() && registration.status !== "unregistered"),
        );
        ...
        <>
            <div className="flex flex-col gap-2">
                {registrations.map((registration) => (
                    <Link
                        href={happeningTypeToPath[registration.happening.type] + "/" + registration.happening.slug}
                        key={registration.happening.slug}
                        className="hover:bg-muted rounded-md p-4 border bg-accent grid grid-cols-3"
                    >
                        <div className="flex flex-col gap-2 col-span-2">
                            <h1 className="line-clamp-1 overflow-hidden font-bold">{registration.happening.title}</h1>
                        ...
                        <div className="flex justify-end items-center">
                            <p className={cn("text-xs font-bold", registrationStatusToColor[registration.status])}>
