...
export default defineType({
  ...
  preview: {
    ...
    prepare({media, title, jobType}) {
      const [subtitle] = JOB_TYPES.flatMap((option) =>
  ...
  fields: [
    ...
    defineField({
      ...
      title: "Tittel",
      description:
        'Tittel på stillingsannonse, f.eks: "Bekk søker nyutdannede utviklere til bla bla bla"',
      validation: (Rule) => Rule.required(),
      type: "string",
    }),
    defineField({
      ...
      title: "Slug (link)",
      description:
        'Unik identifikator for annonsen. Bruk "Generate"-knappen! Ikke skriv inn på egenhånd!',
      type: "slug",
      options: {
        source: "title",
        slugify: (input: string) =>
          slugify(input, {remove: /[*+~.()'"!:@]/g, lower: true, strict: true}),
      },
    }),
    defineField({
      name: "body",
    ...
    defineField({
      ...
      validation: (Rule: ArrayRule<Array<number>>) => [
        Rule.custom((degreeYears) =>
          degreeYears?.every((year) => year >= 1 && year <= 5)
            ? true
            : "Alle årstrinn må være mellom 1 og 5",
        )
          .required()
          .unique(),
      ],
