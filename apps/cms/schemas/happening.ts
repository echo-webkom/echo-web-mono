...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) => Rule.required(),
      description:
        'Unik identifikator for arrangementet. Bruk "Generate"-knappen! Ikke skriv inn på egenhånd!',
      type: "slug",
      ...
      options: {
        ...
        slugify: async (
          ...
          const query =
            'count(*[_type == "happening" && slug.current == $slug]{_id})';
          const params = {slug};
          const {getClient} = context;

          const count = await getClient({apiVersion: "2021-04-10"}).fetch(
            query,
            params,
          );
    ...
    defineField({
      ...
      readOnly: ({value, document}) =>
        !!(
          value &&
          (document?.registrationDate || document?.registrationDeadline)
        ),
    }),
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((registrationDeadline, context) => {
          ...
          if (
            ...
            new Date(document.registrationDate as Date)
          ) {
            return "Påmeldingsfristen må være senere enn påmeldingsdatoen.";
          }

          if (
            new Date(registrationDeadline) > new Date(document.date as Date)
          ) {
    ...
    defineField({
      ...
      readOnly: ({value, document}) =>
        !!(
          value &&
          (document?.studentGroupRegistrationDate || document?.studentGroups)
        ),
    }),
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((studentGroupRegistrationDate, context) => {
          ...
          if (
            ...
            new Date(studentGroupRegistrationDate) >
              new Date(document?.registrationDate as Date)
          ) {
    ...
    defineField({
      ...
      description:
        "Dersom denne er valgt, vil kun undergruppene valgt over kunne melde seg på.",
      type: "boolean",
      initialValue: false,
      hidden: ({document, value}) =>
        !value && !document?.studentGroupRegistrationDate,
    }),
    defineField({
      name: "deductible",
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((logo, context) =>
          context.document?.happeningType === "BEDPRES" &&
          typeof logo === "undefined"
            ? "Må ha logo om det er en bedpres."
            : true,
        ),
      hidden: ({document, value}) =>
        !value && document?.happeningType !== "BEDPRES",
      type: "image",
    }),
    defineField({
      name: "location",
    ...
    defineField({
      ...
      description:
        "Ekstra spørsmål til brukeren på et arrangement (f.eks. hvilken mat, allergier osv...)",
      type: "array",
      of: [
        defineArrayMember({
          type: "reference",
    ...
    defineField({
      ...
      description:
        "Hvor mange plasser som er tildelt hvert trinn på et arrangement.",
      type: "array",
      validation: (Rule) =>
