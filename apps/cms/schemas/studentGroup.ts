...
export default defineType({
  ...
  preview: {
    ...
    prepare({title, groupType}) {
      const [subtitle] = GROUP_TYPES.flatMap((option) =>
        option.value === groupType ? option.title : [],
      );
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) => Rule.required(),
      description:
        'Unik identifikator for studentgruppen. Bruk "Generate"-knappen! Ikke skriv inn på egenhånd!',
      type: "slug",
      options: {
        source: "name",
