...
// webkom group.
func NewAdminMiddleware(authService *service.AuthService, adminAPIKey string) uno.Middleware {
...
func NewSessionMiddleware(authService *service.AuthService) uno.Middleware {
	return func(h http.Handler) http.Handler {
		return uno.Handler(func(ctx *uno.Context) error {
			user, session, err := validateSessionRequest(ctx, authService)
			if err != nil {
...
func NewAdminOrSessionMiddleware(authService *service.AuthService, adminAPIKey string) uno.Middleware {
	return func(h http.Handler) http.Handler {
		return uno.Handler(func(ctx *uno.Context) error {
			if hasValidAdminAPIKey(ctx, adminAPIKey) {
				setAdminAttributes(ctx)
...
// "X-Admin-Key" header.
func hasValidAdminAPIKey(ctx *uno.Context, adminAPIKey string) bool {
	if adminAPIKey == "" {
		return false
	}
	return ctx.HeaderValue(uno.XAdminKeyHeader) == adminAPIKey
}
...
func bearerTokenFromHeader(ctx *uno.Context) (string, error) {
	token := ctx.HeaderValue(uno.AuthorizationHeader)
	if token == "" {
...
func validateSessionRequest(ctx *uno.Context, authService *service.AuthService) (model.User, model.Session, error) {
...
func setAdminAttributes(ctx *uno.Context) {
...
func setSessionContext(ctx *uno.Context, user model.User, session model.Session) {
	nextCtx := WithUser(ctx.Context(), user)
	nextCtx = WithSession(nextCtx, session)
