...
func BannedUsersFromDomainList(users []model.UserWithBanInfo) []UserWithBanInfoResponse {
	...
	for i, user := range users {
		banInfo := &BanInfo{
			...
			CreatedAt: api.FormatISO8601(user.BanInfo.CreatedAt),
			UserID:    user.BanInfo.UserID,
			BannedBy:  user.BanInfo.BannedByID,
			ExpiresAt: api.FormatISO8601(user.BanInfo.ExpiresAt),
		...
		for j, dot := range user.Dots {
			dots[j] = DotInfo{
				ID:        dot.ID,
				Reason:    dot.Reason,
				CreatedAt: api.FormatISO8601(dot.CreatedAt),
				UserID:    dot.UserID,
				ExpiresAt: api.FormatISO8601(dot.ExpiresAt),
				Count:     dot.Count,
