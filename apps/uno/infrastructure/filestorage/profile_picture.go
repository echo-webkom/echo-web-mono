...
func NewProfilePictureStore(ctx context.Context, fileStorage *FileStorage, bucketName string, logger port.Logger) (port.ProfilePictureRepo, error) {
	exists, err := fileStorage.BucketExists(ctx, bucketName)
	if err != nil {
		return nil, ErrProfilePictureBucketNotFound
	}
	...
	return &ProfilePictureRepo{minio: fileStorage, bucketName: bucketName, logger: logger}, nil
}
...
func (p *ProfilePictureRepo) DeleteProfilePicture(ctx context.Context, userID string) error {
	...
	for _, key := range keys {
		if err := p.minio.RemoveObject(ctx, p.bucketName, key, minio.RemoveObjectOptions{}); err != nil {
...
func (p *ProfilePictureRepo) uploadResized(ctx context.Context, key string, img image.Image, maxSize int) error {
	...
	data := buf.Bytes()
	_, err := p.minio.PutObject(ctx, p.bucketName, key, bytes.NewReader(data), int64(len(data)), minio.PutObjectOptions{
